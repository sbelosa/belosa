...
$assertions = [
    ...
    'VIP task metadata fails closed if the separately deployed rules file is missing or incomplete' => str_contains($helper, 'VIP task metadata is missing; the curriculum cannot be opened safely.') && str_contains($helper, 'must contain exactly 30 reviewed rules') && str_contains($helper, "foreach(['starter', 'activator', 'builder', 'leader', 'reactivation'] as \$track_key)"),
...
$rule_assertions = [
    ...
    'message examples and resource fallbacks are connected only to reviewed relevant tasks' => !empty($catalog['starter'][4]['message_example']) && empty($catalog['starter'][13]['message_example']) && empty($catalog['starter'][27]['message_example']) && !empty($catalog['activator'][15]['message_example']) && empty($catalog['activator'][18]['message_example']) && empty($catalog['activator'][25]['message_example']) && !empty($catalog['builder'][15]['message_example']) && !empty($catalog['builder'][22]['message_example']) && empty($catalog['builder'][25]['message_example']) && empty($catalog['leader'][6]['message_example']) && empty($catalog['leader'][26]['message_example']) && empty($catalog['leader'][29]['message_example']) && !empty($catalog['reactivation'][4]['message_example']) && !empty($catalog['reactivation'][8]['message_example']) && empty($catalog['reactivation'][11]['message_example']) && empty($catalog['reactivation'][21]['message_example']) && empty($catalog['reactivation'][25]['message_example']) && empty($catalog['reactivation'][27]['message_example']) && !empty($catalog['starter'][7]['fallback']) && !empty($catalog['leader'][1]['fallback']),
