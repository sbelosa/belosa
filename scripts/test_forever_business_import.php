...
$starter_first_action = forever_business_get_action($starter_member, null, 0, false, $weekday_morning);
...
$rule_assertions = [
    ...
    'planning and preparation tasks do not inflate training or direct-action analytics' => ($catalog['starter'][10]['expected_result_type'] ?? '') === 'planning' && ($catalog['activator'][24]['expected_result_type'] ?? '') === 'planning' && ($catalog['builder'][24]['expected_result_type'] ?? '') === 'planning' && ($catalog['leader'][3]['expected_result_type'] ?? '') === 'planning' && ($catalog['leader'][3]['target'] ?? 0) === 1 && ($catalog['leader'][6]['expected_result_type'] ?? '') === 'planning' && ($catalog['leader'][27]['expected_result_type'] ?? '') === 'planning' && ($catalog['reactivation'][17]['expected_result_type'] ?? '') === 'planning',
    'Starter opens with a concrete FCC bio-link task while completed day one keeps its existing progress key' => ($catalog['starter'][1]['core'] ?? '') === 'Productivity' && ($catalog['starter'][1]['target'] ?? 0) === 1 && ($catalog['starter'][1]['quick_target'] ?? 0) === 1 && ($catalog['starter'][1]['expected_result_type'] ?? '') === 'content' && ($catalog['starter'][1]['allowed_result_types'] ?? []) === ['content'] && count($catalog['starter'][1]['checklist'] ?? []) === 3 && mb_stripos((string) ($catalog['starter'][1]['task_text'] ?? ''), 'FCC link u bio') !== false && str_contains((string) ($catalog['starter'][1]['success_definition'] ?? ''), 'postavljen je i provjeren') && str_contains((string) ($catalog['starter'][1]['fallback'] ?? ''), 'Za lakšu verziju') && empty($catalog['starter'][1]['message_example']) && ($starter_first_action['key'] ?? '') === 'vip26_starter_d01' && ($at_next_midnight_action['key'] ?? '') === 'vip26_starter_d02',
