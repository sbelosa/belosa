...
<!-- Custom code: FC-2026-03-05: BIH Forever Living full styling update form -->
<form id="<?= 'update_biolink_block_' . $row->biolink_block_id ?>" name="update_biolink_" method="post" role="form" data-type="<?= $row->type ?>">
    ...
    <input type="hidden" name="block_type" value="link_forever_living_bih" />
    <input type="hidden" name="biolink_block_id" value="<?= $row->biolink_block_id ?>" />
    <input type="hidden" name="location_url" value="https://www.flpshop.ba/?id=" />
    <input type="hidden" name="border_shadow_offset_x" value="<?= $row->settings->border_shadow_offset_x ?? 0 ?>" />
    <input type="hidden" name="border_shadow_offset_y" value="<?= $row->settings->border_shadow_offset_y ?? 0 ?>" />
    <input type="hidden" name="border_shadow_blur" value="<?= $row->settings->border_shadow_blur ?? 20 ?>" />
    <input type="hidden" name="border_shadow_spread" value="<?= $row->settings->border_shadow_spread ?? 1 ?>" />

    <div class="notification-container"></div>

    <div class="alert alert-info">
        <i class="fas fa-fw fa-circle-info mr-1"></i> <?= l('create_biolink_link_forever_living_bih_modal.info') ?>
    </div>

    <div class="form-group">
        <label for="<?= 'link_name_' . $row->biolink_block_id ?>"><i class="fa fa-fw fa-signature fa-sm text-muted mr-1"></i> <?= l('biolink_link.name') ?></label>
    ...
    <div class="form-group custom-control custom-switch">
        <input id="<?= 'link_open_in_new_tab_' . $row->biolink_block_id ?>" name="open_in_new_tab" type="checkbox" class="custom-control-input" <?= $row->settings->open_in_new_tab ? 'checked="checked"' : null ?>>
    ...
    <div class="collapse" data-parent="<?= '#update_biolink_block_' . $row->biolink_block_id ?>" id="<?= 'border_container_' . $row->biolink_block_id ?>">
        ...
        <div class="form-group">
            ...
            <input id="<?= 'block_border_color_' . $row->biolink_block_id ?>" type="hidden" name="border_color" class="form-control" value="<?= $row->settings->border_color ?? '#000000' ?>" required="required" />
        ...
        <div class="form-group">
            ...
            <div class="row btn-group-toggle m-n2" data-toggle="buttons">
                <?php foreach(['straight', 'round', 'rounded'] as $border_radius): ?>
                    <div class="p-2 col-4">
                        <label class="btn btn-light btn-block font-size-small mb-0 text-truncate <?= ($row->settings->border_radius ?? null) == $border_radius ? 'active"' : null?>">
                            <input type="radio" name="border_radius" value="<?= $border_radius ?>" class="custom-control-input" <?= ($row->settings->border_radius ?? null) == $border_radius ? 'checked="checked"' : null?> />
    ...
    <div class="collapse" data-parent="<?= '#update_biolink_block_' . $row->biolink_block_id ?>" id="<?= 'border_shadow_container_' . $row->biolink_block_id ?>">
        ...
        <div class="form-group">
            ...
            <input id="<?= 'block_border_shadow_color_' . $row->biolink_block_id ?>" type="hidden" name="border_shadow_color" class="form-control" value="<?= $row->settings->border_shadow_color ?? '#000000' ?>" required="required" />
