...
<!-- Custom code: FC-2026-02-27: pets ai chatbot update form -->
<form name="update_biolink_" method="post" role="form">
    ...
    <?php if (\Altum\Authentication::is_admin()): ?>
        <div class="form-group">
            <label for="<?= 'custom_html_chatbot_pets_html_' . $row->biolink_block_id ?>"><i class="fa fa-fw fa-code fa-sm text-muted mr-1"></i> <?= l('create_biolink_custom_html_chatbot_pets_modal.html') ?></label>
            <textarea id="<?= 'custom_html_chatbot_pets_html_' . $row->biolink_block_id ?>" name="html" class="form-control" maxlength="<?= $data->biolink_blocks['custom_html_chatbot_pets']['max_length'] ?>"><?= $row->settings->html ?></textarea>
        </div>
    <?php endif; ?>
    ...
    <div class="collapse" id="<?= 'display_settings_container_' . $row->biolink_block_id ?>">
        ...
        <div class="form-group">
            ...
            <small class="form-text text-muted"><?= l('biolink_link.display_countries_help') ?></small>
        </div>

        <div class="form-group">
            <label for="<?= 'link_display_devices_' . $row->biolink_block_id ?>"><i class="fa fa-fw fa-laptop fa-sm text-muted mr-1"></i> <?= l('biolink_link.display_devices') ?></label>
            ...
            <small class="form-text text-muted"><?= l('biolink_link.display_devices_help') ?></small>
        </div>

        <div class="form-group">
            <label for="<?= 'link_display_languages_' . $row->biolink_block_id ?>"><i class="fa fa-fw fa-language fa-sm text-muted mr-1"></i> <?= l('biolink_link.display_languages') ?></label>
            ...
            <small class="form-text text-muted"><?= l('biolink_link.display_languages_help') ?></small>
        </div>

        <div class="form-group">
            <label for="<?= 'link_display_operating_systems_' . $row->biolink_block_id ?>"><i class="fa fa-fw fa-window-restore fa-sm text-muted mr-1"></i> <?= l('biolink_link.display_operating_systems') ?></label>
            ...
            <small class="form-text text-muted"><?= l('biolink_link.display_operating_systems_help') ?></small>
        </div>
    </div>
