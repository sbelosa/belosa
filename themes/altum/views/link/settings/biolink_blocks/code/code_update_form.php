...
<?php
$shiki_languages = ['text','plaintext','html','css','scss','sass','less','javascript','typescript','json','jsonc','xml','yaml','markdown','mdx','php','python','ruby','go','java','kotlin','scala','groovy','rust','swift','c','cpp','objective-c','csharp','bash','shellscript','zsh','fish','powershell','perl','lua','sql','graphql','dockerfile','nginx','ini','toml','dotenv','diff','makefile','cmake','regex','git-commit','git-rebase','git-ignore'];
?>
...
<form id="<?= 'update_biolink_block_' . $row->biolink_block_id ?>" name="update_biolink_" method="post" role="form" data-type="<?= $row->type ?>">
    ...
    <input type="hidden" name="block_type" value="code" />
    <input type="hidden" name="biolink_block_id" value="<?= $row->biolink_block_id ?>" />

    <div class="notification-container"></div>

    <div class="form-group">
        <label for="<?= 'code_code_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-code fa-sm text-muted mr-1"></i> <?= l('biolink_code.code') ?></label>
        <textarea id="<?= 'code_code_' . $row->biolink_block_id ?>" class="form-control" name="code" maxlength="10000"><?= e($row->settings->code) ?></textarea>
    </div>

    <div class="form-group">
        <label for="<?= 'code_language_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-code-branch fa-sm text-muted mr-1"></i> <?= l('biolink_code.language') ?></label>
        <select id="<?= 'code_language_' . $row->biolink_block_id ?>" name="code_language" class="custom-select">
            <?php foreach ($shiki_languages as $language) : ?>
                <option value="<?= $language ?>" <?= $row->settings->code_language == $language ? 'selected="selected"' : null ?>><?= $language ?></option>
            <?php endforeach ?>
        </select>
    </div>

    <div class="form-group">
        <label for="<?= 'code_theme_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-palette fa-sm text-muted mr-1"></i><?= l('biolink_code.theme') ?></label>
        <select id="<?= 'code_theme_' . $row->biolink_block_id ?>" name="theme" class="custom-select">
            <?php foreach (['github-light','github-dark','github-dark-dimmed','dark-plus','light-plus','dracula','dracula-soft','nord','one-dark-pro','material-theme','material-theme-darker','material-theme-lighter','monokai','slack-dark','slack-ochin','solarized-light','solarized-dark','tokyo-night','tokyo-night-light','rose-pine','rose-pine-dawn','rose-pine-moon'] as $theme): ?>
                <option value="<?= $theme ?>" <?= $row->settings->theme == $theme ? 'selected="selected"' : null ?>><?= $theme ?></option>
            <?php endforeach ?>
        </select>
    </div>
