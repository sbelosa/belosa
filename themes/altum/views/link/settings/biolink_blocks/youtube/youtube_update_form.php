...
<form id="<?= 'update_biolink_block_' . $row->biolink_block_id ?>" name="update_biolink_" method="post" role="form" data-type="<?= $row->type ?>">
    ...
    <div class="form-group">
        <label for="<?= 'youtube_title_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-heading fa-sm text-muted mr-1"></i> <?= l('global.title') ?></label>
        <input id="<?= 'youtube_title_' . $row->biolink_block_id ?>" type="text" class="form-control" name="title" value="<?= htmlspecialchars((string) ($row->settings->title ?? ''), ENT_QUOTES, 'UTF-8') ?>" maxlength="256" />
    </div>

    <div class="form-group">
        <label><i class="fas fa-fw fa-paint-brush fa-sm text-muted mr-1"></i> <?= l('biolink_link.text_color') ?></label>
        <input id="<?= 'youtube_text_color_' . $row->biolink_block_id ?>" type="hidden" name="text_color" class="form-control" value="<?= htmlspecialchars($title_color, ENT_QUOTES, 'UTF-8') ?>" required="required" />
        <div class="text_color_pickr"></div>
    </div>

    <div class="form-group">
        <label for="<?= 'youtube_font_size_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-font fa-sm text-muted mr-1"></i> <?= l('link.settings.font_size') ?></label>
        <input id="<?= 'youtube_font_size_' . $row->biolink_block_id ?>" type="number" min="12" max="40" class="form-control" name="font_size" value="<?= $title_font_size ?>" />
    </div>
