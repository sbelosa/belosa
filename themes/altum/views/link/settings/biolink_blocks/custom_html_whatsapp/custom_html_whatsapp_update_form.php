...
<form id="<?= 'update_biolink_block_' . $row->biolink_block_id ?>" name="update_biolink_" method="post" role="form" data-type="<?= $row->type ?>">
    ...
    <div class="form-group">
        <label for="<?= 'custom_html_whatsapp_title_' . $row->biolink_block_id ?>"><i class="fa fa-fw fa-heading fa-sm text-muted mr-1"></i> <?= l('link.biolink.blocks.heading') ?></label>
        <input id="<?= 'custom_html_whatsapp_title_' . $row->biolink_block_id ?>" name="title" class="form-control" value="<?= $row->settings->title ?>" maxlength="256" required="required">
    </div>

    <div class="form-group">
        <label for="<?= 'custom_html_whatsapp_icon_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-icons fa-sm text-muted mr-1"></i> <?= l('global.icon') ?></label>
        <input id="<?= 'custom_html_whatsapp_icon_' . $row->biolink_block_id ?>" type="text" name="icon" class="form-control" value="<?= $row->settings->icon ?? 'fab fa-whatsapp' ?>" placeholder="<?= l('global.icon_placeholder') ?>" />
        <small class="form-text text-muted"><?= l('global.icon_help') ?></small>
    </div>

    <div class="form-group">
        <label for="<?= 'custom_html_whatsapp_text_color_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-paint-brush fa-sm text-muted mr-1"></i> <?= l('biolink_link.text_color') ?></label>
        <input id="<?= 'custom_html_whatsapp_text_color_' . $row->biolink_block_id ?>" type="hidden" name="text_color" class="form-control" value="<?= $row->settings->text_color ?? '#000000' ?>" required="required" />
        <div class="text_color_pickr"></div>
    </div>

    <div class="form-group">
        <label for="<?= 'custom_html_whatsapp_text_alignment_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-align-center fa-sm text-muted mr-1"></i> <?= l('biolink_link.text_alignment') ?></label>
        <div class="row btn-group-toggle m-n2" data-toggle="buttons">
            <?php foreach(['center', 'justify', 'left', 'right'] as $text_alignment): ?>
                <div class="p-2 col-6">
                    <label class="btn btn-light btn-block font-size-small mb-0 text-truncate <?= ($row->settings->text_alignment ?? 'center') == $text_alignment ? 'active"' : null?>">
    ...
    <div class="form-group">
        <label for="<?= 'custom_html_whatsapp_background_color_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-fill fa-sm text-muted mr-1"></i> <?= l('biolink_link.background_color') ?></label>
        <input id="<?= 'custom_html_whatsapp_background_color_' . $row->biolink_block_id ?>" type="hidden" name="background_color" class="form-control" value="<?= $row->settings->background_color ?? '#ffffff' ?>" required="required" />
    ...
    <div class="collapse" data-parent="<?= '#update_biolink_block_' . $row->biolink_block_id ?>" id="<?= 'border_container_' . $row->biolink_block_id ?>">
        <div class="form-group" data-range-counter data-range-counter-suffix="px">
            <label for="<?= 'custom_html_whatsapp_border_width_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-border-style fa-sm text-muted mr-1"></i> <?= l('biolink_link.border_width') ?></label>
            <input id="<?= 'custom_html_whatsapp_border_width_' . $row->biolink_block_id ?>" type="range" min="0" max="5" class="form-control-range" name="border_width" value="<?= $row->settings->border_width ?? 0 ?>" required="required" />
        </div>

        <div class="form-group">
            <label for="<?= 'custom_html_whatsapp_border_color_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-fill fa-sm text-muted mr-1"></i> <?= l('biolink_link.border_color') ?></label>
            <input id="<?= 'custom_html_whatsapp_border_color_' . $row->biolink_block_id ?>" type="hidden" name="border_color" class="form-control" value="<?= $row->settings->border_color ?? '#ffffff' ?>" required="required" />
            <div class="border_color_pickr"></div>
        </div>

        <div class="form-group">
            <label for="<?= 'custom_html_whatsapp_border_radius_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-border-all fa-sm text-muted mr-1"></i> <?= l('biolink_link.border_radius') ?></label>
            <div class="row btn-group-toggle m-n2" data-toggle="buttons">
                <div class="p-2 col-4">
                    <label class="btn btn-light btn-block font-size-small mb-0 text-truncate <?= ($row->settings->border_radius ?? 'rounded') == 'straight' ? 'active"' : null?>">
                ...
                <div class="p-2 col-4">
                    <label class="btn btn-light btn-block font-size-small mb-0 text-truncate <?= ($row->settings->border_radius ?? 'rounded') == 'round' ? 'active' : null?>">
                ...
                <div class="p-2 col-4">
                    <label class="btn btn-light btn-block font-size-small mb-0 text-truncate <?= ($row->settings->border_radius ?? 'rounded') == 'rounded' ? 'active' : null?>">
        ...
        <div class="form-group">
            <label for="<?= 'custom_html_whatsapp_border_style_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-border-none fa-sm text-muted mr-1"></i> <?= l('biolink_link.border_style') ?></label>
            <div class="row btn-group-toggle m-n2" data-toggle="buttons">
    ...
    <div class="collapse" data-parent="<?= '#update_biolink_block_' . $row->biolink_block_id ?>" id="<?= 'border_shadow_container_' . $row->biolink_block_id ?>">
        <div class="form-group">
            <label for="<?= 'custom_html_whatsapp_border_shadow_style_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-cloud-sun fa-sm text-muted mr-1"></i> <?= l('biolink_link.border_shadow_style') ?></label>
            <div class="row btn-group-toggle m-n2" data-toggle="buttons">
                <?php foreach(['none', 'subtle', 'strong', 'hard'] as $border_shadow_style): ?>
                    <div class="p-2 col-4">
                        <label class="btn btn-light btn-block font-size-small mb-0 text-truncate <?= ($row->settings->border_shadow_style ?? 'subtle') == $border_shadow_style ? 'active"' : null?>">
                            <input type="radio" name="border_shadow_style" value="<?= $border_shadow_style ?>" class="custom-control-input" <?= ($row->settings->border_shadow_style ?? 'subtle') == $border_shadow_style ? 'checked="checked"' : null?> />
        ...
        <div class="form-group">
            <label for="<?= 'custom_html_whatsapp_border_shadow_color_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-fill fa-sm text-muted mr-1"></i> <?= l('biolink_link.border_shadow_color') ?></label>
            <input id="<?= 'custom_html_whatsapp_border_shadow_color_' . $row->biolink_block_id ?>" type="hidden" name="border_shadow_color" class="form-control" value="<?= $row->settings->border_shadow_color ?? '#00000010' ?>" required="required" />
    ...
    <div class="form-group">
        <label for="<?= 'custom_html_whatsapp_message_' . $row->biolink_block_id ?>"><i class="fa fa-fw fa-comment fa-sm text-muted mr-1"></i> <?= l('create_biolink_custom_html_whatsapp_modal.html') ?></label>
        <textarea id="<?= 'custom_html_whatsapp_message_' . $row->biolink_block_id ?>" name="message" class="form-control" maxlength="<?= $data->biolink_blocks['custom_html_whatsapp']['max_length'] ?>"><?= $row->settings->message ?></textarea>
    </div>
