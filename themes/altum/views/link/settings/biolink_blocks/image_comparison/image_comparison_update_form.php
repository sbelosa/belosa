...
<form id="<?= 'update_biolink_block_' . $row->biolink_block_id ?>" name="update_biolink_" method="post" role="form" data-type="<?= $row->type ?>" enctype="multipart/form-data">
    ...
    <input type="hidden" name="block_type" value="image_comparison" />
    <input type="hidden" name="biolink_block_id" value="<?= $row->biolink_block_id ?>" />

    <div class="notification-container"></div>

    <div class="form-group">
        <label for="<?= 'block_before_text_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-pen fa-sm text-muted mr-1"></i> <?= l('biolink_image_comparison.before_text') ?></label>
        <input id="<?= 'block_before_text_' . $row->biolink_block_id ?>" type="text" name="before_text" class="form-control" value="<?= $row->settings->before_text ?>" maxlength="128" />
    </div>
    ...
    <div class="form-group" data-file-image-input-wrapper data-file-input-wrapper-size-limit="<?= settings()->links->image_size_limit ?>" data-file-input-wrapper-size-limit-error="<?= sprintf(l('global.error_message.file_size_limit'), settings()->links->image_size_limit) ?>">
        <label for="<?= 'image_comparison_before_image_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-image fa-sm text-muted mr-1"></i> <?= l('biolink_image_comparison.before_image') ?></label>
        <?= include_view(THEME_PATH . 'views/partials/custom_file_image_input.php', [
            'id'=> 'image_comparison_before_image_' . $row->biolink_block_id,
            'uploads_file_key' => 'block_images',
            'file_key' => 'before_image',
            'already_existing_image' => $row->settings->before_image,
            'image_container' => 'before_image',
            'accept' => \Altum\Uploads::array_to_list_format($data->biolink_blocks['image_comparison']['whitelisted_image_extensions']),
            'input_data' => 'data-crop data-aspect-ratio="1"'
        ]) ?>
        <small class="form-text text-muted"><?= sprintf(l('global.accessibility.whitelisted_file_extensions'), \Altum\Uploads::array_to_list_format($data->biolink_blocks['image_comparison']['whitelisted_image_extensions'])) . ' ' . sprintf(l('global.accessibility.file_size_limit'), settings()->links->image_size_limit) ?></small>
    </div>

    <div class="form-group">
        <label for="<?= 'image_comparison_before_image_alt_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-comment-dots fa-sm text-muted mr-1"></i> <?= l('biolink_link.image_alt') ?></label>
        <input id="<?= 'image_comparison_before_image_alt_' . $row->biolink_block_id ?>" type="text" class="form-control" name="before_image_alt" value="<?= $row->settings->before_image_alt ?>" maxlength="100" />
        <small class="form-text text-muted"><?= l('biolink_link.image_alt_help') ?></small>
    </div>

    <div class="form-group">
        <label for="<?= 'block_after_text_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-pen fa-sm text-muted mr-1"></i> <?= l('biolink_image_comparison.after_text') ?></label>
        <input id="<?= 'block_after_text_' . $row->biolink_block_id ?>" type="text" name="after_text" class="form-control" value="<?= $row->settings->after_text ?>" maxlength="128" />
    </div>

    <div class="form-group" data-file-image-input-wrapper data-file-input-wrapper-size-limit="<?= settings()->links->image_size_limit ?>" data-file-input-wrapper-size-limit-error="<?= sprintf(l('global.error_message.file_size_limit'), settings()->links->image_size_limit) ?>">
        <label for="<?= 'image_comparison_after_image_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-image fa-sm text-muted mr-1"></i> <?= l('biolink_image_comparison.after_image') ?></label>
        <?= include_view(THEME_PATH . 'views/partials/custom_file_image_input.php', [
                'id'=> 'image_comparison_after_image_' . $row->biolink_block_id,
                'uploads_file_key' => 'block_images',
                'file_key' => 'after_image',
                'already_existing_image' => $row->settings->after_image,
                'image_container' => 'after_image',
                'accept' => \Altum\Uploads::array_to_list_format($data->biolink_blocks['image_comparison']['whitelisted_image_extensions']),
                'input_data' => 'data-crop data-aspect-ratio="1"'
        ]) ?>
        <small class="form-text text-muted"><?= sprintf(l('global.accessibility.whitelisted_file_extensions'), \Altum\Uploads::array_to_list_format($data->biolink_blocks['image_comparison']['whitelisted_image_extensions'])) . ' ' . sprintf(l('global.accessibility.file_size_limit'), settings()->links->image_size_limit) ?></small>
    ...
    <div class="form-group">
        <label for="<?= 'image_comparison_after_image_alt_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-comment-dots fa-sm text-muted mr-1"></i> <?= l('biolink_link.image_alt') ?></label>
        <input id="<?= 'image_comparison_after_image_alt_' . $row->biolink_block_id ?>" type="text" class="form-control" name="after_image_alt" value="<?= $row->settings->after_image_alt ?>" maxlength="100" />
        <small class="form-text text-muted"><?= l('biolink_link.image_alt_help') ?></small>
    </div>
