...
<div id="<?= 'biolink_block_id_' . $data->link->biolink_block_id ?>" data-biolink-block-id="<?= $data->link->biolink_block_id ?>" data-biolink-block-type="<?= $data->link->type ?>" class="col-12 my-<?= $data->biolink->settings->block_spacing ?? '2' ?>">
    <div class="d-flex flex-column align-items-center">
        ...
        <?php endif ?>
            ...
            <img src="<?= !empty($data->link->settings->image) ? \Altum\Uploads::get_full_url('avatars') . $data->link->settings->image : null ?>" class="link-image <?= 'link-avatar-' . $avatar_border_radius ?> <?= $data->link->location_url ? ($data->biolink->settings->hover_animation ?? 'smooth') != 'false' ? 'link-hover-animation-' . ($data->biolink->settings->hover_animation ?? 'smooth') : null : null ?>" style="<?= !empty($data->link->settings->image) ? null : 'display: none;' ?>width: <?= $avatar_size ?>px; height: <?= $avatar_size ?>px; border-width: <?= $data->link->settings->border_width ?? 0 ?>px; border-color: <?= $data->link->settings->border_color ?? '#ffffff' ?>; border-style: <?= $data->link->settings->border_style ?? 'solid' ?>; object-fit: <?= $avatar_object_fit ?>; <?= \Altum\Link::get_processed_box_shadow_style($data->link->settings) ?>" alt="<?= $data->link->settings->image_alt ?? '' ?>" loading="lazy" data-border-width data-border-avatar-radius data-border-style data-border-color data-border-shadow data-avatar />
