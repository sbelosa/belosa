...
<?php
/* Custom code: FC-2026-03-08: unresolved feedback tickets counter for admin sidebar */
$admin_unresolved_feedback_tickets = 0;
try {
    ...
    if($has_feedback_tickets_table_result && $has_feedback_tickets_table_result->num_rows) {
        $admin_unresolved_feedback_tickets = (int) (database()->query("SELECT COUNT(*) AS `total` FROM `feedback_tickets` WHERE `status` != 'closed'")->fetch_object()->total ?? 0);
    }
...
<section class="admin-sidebar">
    ...
    <div class="admin-sidebar-links-wrapper">
        <ul class="admin-sidebar-links">
            ...
            <!-- Custom code: FC-2026-03-08: admin feedback tickets menu -->
            <li class="<?= in_array(\Altum\Router::$controller, ['AdminFeedbackTickets']) ? 'active' : null ?>">
                <a class="nav-link text-truncate" href="<?= url('admin/feedback-tickets') ?>">
                    <i class="fas fa-fw fa-sm fa-comments mr-2"></i> <?= l('admin_feedback_tickets.menu') ?>
                    <?php if($admin_unresolved_feedback_tickets): ?>
                        <span class="badge badge-danger ml-2"><?= nr($admin_unresolved_feedback_tickets) ?></span>
                    <?php endif ?>
                </a>
            </li>
            <!-- /Custom code: FC-2026-03-08 -->
