...
<div>
    ...
    <div class="<?= !in_array(settings()->license->type, ['Extended License', 'extended']) ? 'container-disabled' : null ?>">
        ...
        <div class="form-group custom-control custom-switch">
            <input id="is_enabled" name="is_enabled" type="checkbox" class="custom-control-input" <?= settings()->plisio->is_enabled ? 'checked="checked"' : null?>>
            <label class="custom-control-label" for="is_enabled"><?= l('admin_settings.plisio.is_enabled') ?></label>
        </div>

        <div class="form-group">
            <label for="secret_key"><?= l('admin_settings.plisio.secret_key') ?></label>
            <input id="secret_key" type="text" name="secret_key" class="form-control" value="<?= settings()->plisio->secret_key ?>" />
        </div>
        ...
        <div class="form-group">
            ...
            <select id="accepted_cryptocurrencies" name="accepted_cryptocurrencies[]" class="custom-select" multiple="multiple">
                ...
                <option value="<?= $token ?>" <?= in_array($token, settings()->plisio->accepted_cryptocurrencies ?? []) ? 'selected="selected"' : null ?>><?= $token . ' - ' . $cryptocurrency['name'] ?></option>
                <?php endforeach ?>
            </select>
        </div>
        ...
        <div class="form-group">
            ...
            <select id="default_cryptocurrency" name="default_cryptocurrency" class="custom-select">
                <?php foreach($cryptocurrencies as $token => $cryptocurrency): ?>
                    <option value="<?= $token ?>" <?= $token == settings()->plisio->default_cryptocurrency ? 'selected="selected"' : null ?>><?= $token . ' - ' . $cryptocurrency['name'] ?></option>
                <?php endforeach ?>
            </select>
        </div>
                ...
                <?php foreach((array) settings()->payment->currencies as $currency => $currency_data): ?>
                    <div class="col-12 col-lg-4">
                        <div class="custom-control custom-checkbox my-2">
                            <input id="<?= 'currency_' . $currency ?>" name="currencies[]" value="<?= $currency ?>" type="checkbox" class="custom-control-input" <?= in_array($currency, settings()->plisio->currencies ?? []) ? 'checked="checked"' : null ?>>
        ...
        <div class="form-group">
            ...
            <input type="text" id="webhook_url" value="<?= SITE_URL . 'webhook-plisio' ?>" class="form-control" onclick="this.select();" readonly="readonly" />
        </div>
    </div>
</div>
