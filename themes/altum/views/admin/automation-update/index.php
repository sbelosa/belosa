...
<form action="" method="post" role="form">
            ...
            <div class="row">
                ...
                <div class="col-lg-6">
                    <div class="bg-gray-100 rounded p-3 h-100">
                        ...
                        <div class="small text-muted"><?= sprintf(l('global.variables'), '<code data-copy>' . implode('</code>, <code data-copy>', ['{{USER:NAME}}', '{{USER:EMAIL}}', '{{SALES_LINKS_PAGE}}', '{{FCC_VIDEO_URL}}', '{{USER:LOGIN_LINK}}', '{{WEBSITE_TITLE}}']) . '</code>') ?></div>
                    </div>
                </div>
            </div>
            ...
            <div class="row mt-3">
                <div class="col-lg-12">
                    <div class="form-group mb-0">
                        <label for="video_url"><i class="fas fa-fw fa-sm fa-circle-play text-muted mr-1"></i> <?= l('admin_automation_update.video_url') ?></label>
                        <input type="url" id="video_url" name="video_url" class="form-control" value="<?= e($data->video_url) ?>" placeholder="https://vimeo.com/..." />
                        <small class="form-text text-muted"><?= l('admin_automation_update.video_url_help') ?></small>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="card mb-4">
        <div class="card-body">
            <h2 class="h5 mb-3"><i class="fas fa-fw fa-paper-plane text-muted mr-1"></i> <?= l('admin_automation_update.test_title') ?></h2>

            <div class="row align-items-end">
                <div class="col-lg-4">
                    <div class="form-group">
                        <label for="preview_step_id"><?= l('admin_automation_update.test_step') ?></label>
                        <select id="preview_step_id" name="preview_step_id" class="custom-select">
                            <?php foreach($data->steps as $step): ?>
                                <option value="<?= $step->automation_step_id ?>" <?= $data->preview_step_id == $step->automation_step_id ? 'selected="selected"' : null ?>><?= sprintf(l('admin_automation_update.step_header'), $step->step_order) ?></option>
                            <?php endforeach ?>
                        </select>
                    </div>
                </div>
                ...
                <div class="col-lg-5">
                    <div class="form-group">
                        <label for="preview_email"><?= l('admin_automation_update.test_email') ?></label>
                        <input type="email" id="preview_email" name="preview_email" class="form-control <?= \Altum\Alerts::has_field_errors('preview_email') ? 'is-invalid' : null ?>" value="<?= e($data->preview_email) ?>" placeholder="<?= l('global.email_placeholder') ?>" />
                        <?= \Altum\Alerts::output_field_error('preview_email') ?>
                        <small class="form-text text-muted"><?= l('admin_automation_update.test_help') ?></small>
                    </div>
                </div>

                <div class="col-lg-3">
                    <button type="submit" name="preview" value="1" class="btn btn-outline-primary btn-block mb-2"><?= l('admin_automation_update.test_send') ?></button>
                    <button type="submit" name="preview_all" value="1" class="btn btn-outline-secondary btn-block mb-3"><?= l('admin_automation_update.test_send_all') ?></button>
                </div>
            </div>
        </div>
    </div>
