    ...
    <div class="card-body">
        ...
        <form action="" method="post" role="form" enctype="multipart/form-data">
            ...
            <div class="form-group">
             ...
             <!-- Custom code -->
            <div class="form-group">
                <label for="forever_id"><i class="fa fa-fw fa-sm fa-id-card text-muted mr-1"></i> <?= l('admin_users.main.forever_id') ?></label>
                <input id="forever_id" type="text" name="user_meta[foreverId]" class="form-control <?= \Altum\Alerts::has_field_errors('forever_id') ? 'is-invalid' : null ?>" value="<?= isset($data->user_meta->foreverId) ? $data->user_meta->foreverId : null ?>" />
                <?= \Altum\Alerts::output_field_error('forever_id') ?>
            </div>

            <div class="form-group">
                <label for="phone"><i class="fa fa-fw fa-sm fa-phone text-muted mr-1"></i> <?= l('admin_users.main.phone') ?></label>
                <input id="phone" type="text" name="user_meta[phone]" class="form-control <?= \Altum\Alerts::has_field_errors('phone') ? 'is-invalid' : null ?>" value="<?= isset($data->user_meta->phone) ? $data->user_meta->phone : null ?>" />
                <?= \Altum\Alerts::output_field_error('phone') ?>
            </div>

            <div class="form-group">
                <label for="address"><i class="fa fa-fw fa-sm fa-map-marker-alt text-muted mr-1"></i> <?= l('admin_users.main.address') ?></label>
                <input id="address" type="text" name="user_meta[address]" class="form-control <?= \Altum\Alerts::has_field_errors('address') ? 'is-invalid' : null ?>" value="<?= isset($data->user_meta->address) ? $data->user_meta->address : null ?>" />
                <?= \Altum\Alerts::output_field_error('address') ?>
            </div>

            <div class="form-group">
                <label for="zip"><i class="fa fa-fw fa-sm fa-map-marker-alt text-muted mr-1"></i> <?= l('admin_users.main.zip') ?></label>
                <input id="zip" type="text" name="user_meta[zip]" class="form-control <?= \Altum\Alerts::has_field_errors('zip') ? 'is-invalid' : null ?>" value="<?= isset($data->user_meta->zip) ? $data->user_meta->zip : null ?>" />
                <?= \Altum\Alerts::output_field_error('zip') ?>
            </div>

            <div class="form-group">
                <label for="city"><i class="fa fa-fw fa-sm fa-map-marker-alt text-muted mr-1"></i> <?= l('admin_users.main.city') ?></label>
                <input id="city" type="text" name="user_meta[city]" class="form-control <?= \Altum\Alerts::has_field_errors('city') ? 'is-invalid' : null ?>" value="<?= isset($data->user_meta->city) ? $data->user_meta->city : null ?>" />
                <?= \Altum\Alerts::output_field_error('city') ?>
            </div>

            <div class="form-group">
                <label for="country"><i class="fa fa-fw fa-sm fa-map-marker-alt text-muted mr-1"></i> <?= l('admin_users.main.country') ?></label>
                <input id="country" type="text" name="user_meta[country]" class="form-control <?= \Altum\Alerts::has_field_errors('country') ? 'is-invalid' : null ?>" value="<?= isset($data->user_meta->country) ? $data->user_meta->country : null ?>" />
                <?= \Altum\Alerts::output_field_error('country') ?>
            </div>
            <!-- /Custom code -->
            ...
            <!-- Custom code -->
            <div class="form-group">
                <label for="card_status"><i class="fa fa-fw fa-sm fa-credit-card text-muted mr-1"></i> <?= l('admin_users.main.card_status') ?></label>
                <select id="card_status" name="user_meta[card_status]" class="custom-select">
                    <option value="0" <?= !isset($data->user_meta->card_status) || (isset($data->user_meta->card_status) && $data->user_meta->card_status == 0) ? 'selected="selected"' : null ?>><?= l('admin_users.main.card_status_in_progress') ?></option>
                    <option value="1" <?= isset($data->user_meta->card_status) && $data->user_meta->card_status == 1 ? 'selected="selected"' : null ?>><?= l('admin_users.main.card_status_sent') ?></option>                    
                </select>
                <?php if (isset($data->user_meta->send_card_email)): ?>
                    <small class="form-text text-muted"><?= l('admin_users.main.card_sent_notified') ?></small>
                <?php endif; ?>
            </div>
            ...
            <?php if (!isset($data->user_meta->send_card_email)): ?>                
                <div class="form-group">
                    <div class="custom-control custom-checkbox my-2">
                        <input id="send_card_email" name="user_meta[send_card_email]" type="checkbox" class="custom-control-input">
                        <label class="custom-control-label" for="send_card_email">
                            <span class="text-muted"><?= l('admin_users.main.card_sent_notification') ?></span>
                        </label>
                    </div>
                </div>                            
            <?php endif; ?>
