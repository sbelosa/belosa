    ...
    <div class="card-body">
        ...
        <div class="d-flex justify-content-between align-items-center mb-3">
            <div class="d-flex align-items-center">
                <h3 class="h5 text-truncate m-0"><?= isset($_GET['continent_code']) ? sprintf(l('subscribers_statistics.country_from_continent_code'), get_continent_from_continent_code($data->continent_code)) : l('global.countries') ?></h3>

                <div class="ml-2">
                    <span data-toggle="tooltip" title="<?= l('subscribers_statistics.country_help') ?>">
            ...
            <div class="d-flex align-items-center col-auto p-0">
                <div class="dropdown">
                    ...
                    <div class="dropdown-menu dropdown-menu-right d-print-none">
                        <a href="<?= url('subscribers-statistics?' .  \Altum\Router::$original_request_query . '&export=csv') ?>" target="_blank" class="dropdown-item">
                            <i class="fas fa-fw fa-sm fa-file-csv mr-2"></i> <?= sprintf(l('global.export_to'), 'CSV') ?>
                        </a>
                        <a href="<?= url('subscribers-statistics?' .  \Altum\Router::$original_request_query . '&export=json') ?>" target="_blank" class="dropdown-item <?= $this->user->plan_settings->export->json ? null : 'disabled pointer-events-all' ?>" <?= $this->user->plan_settings->export->json ? null : get_plan_feature_disabled_info() ?>>
        ...
        <?php else: ?>
            ...
            <?php foreach($data->rows as $row): ?>
                ...
                <?php $countries_map[$row->country_code] = ['subscribers' => $row->total]; ?>
                    ...
                    <div class="d-flex justify-content-between mb-1">
                        <div class="text-truncate">
                            ...
                            <?php if($row->country_code): ?>
                                <a href="<?= url('subscribers-statistics?' . $data->website_url_query . '&type=city_name&country_code=' . $row->country_code . '&start_date=' . $data->datetime['start_date'] . '&end_date=' . $data->datetime['end_date']) ?>" title="<?= $row->country_code ?>" class=""><?= get_country_from_country_code($row->country_code) ?></a>
                            <?php else: ?>
        ...
        new svgMap({
            ...
            data: {
                data: {
                    subscribers: {
                        name: '',
                        format: '{0} <?= l('websites.subscriber') ?>',
                        thousandSeparator: thousands_separator,
                    },
                },
                applyData: 'subscribers',
