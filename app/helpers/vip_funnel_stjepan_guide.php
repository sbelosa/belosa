...
function vip_funnel_get_stjepan_forever_card_club_guide_payload($user = null, array $options = []): array {
    $user_id = (int) ($user->user_id ?? 0);

    if($user_id > 0 && (!isset($user->billing) || !isset($user->referral_key))) {
        $fresh_user = db()
    ...
    return stjepan_fcc_guide_build_payload($user, array_merge([
        'slug' => 'stjepan-forever-card-club-vodic',
        'contact_email' => 'info@forevercard.club',
        'facebook_pixel_id' => '238225369103006',
    ], vip_funnel_to_array($options)));
}

function vip_funnel_maybe_create_stjepan_forever_card_club_guide($user = null, string $requested_slug = '') {
    $requested_slug = vip_funnel_slugify($requested_slug, '');
    if($requested_slug !== 'stjepan-forever-card-club-vodic' || !vip_funnel_studio_schema_is_ready()) {
        return null;
    }

    $user_id = (int) ($user->user_id ?? 0);
    $email = trim((string) ($user->email ?? ''));
    if($user_id !== 555 && strcasecmp($email, 'info@forevercard.club') !== 0) {
        return null;
    }
    ...
    $existing = vip_funnel_studio_get_funnel_row_by_slug($user_id, 'stjepan-forever-card-club-vodic');
    if($existing) {
        return $existing;
    }

    $payload = vip_funnel_get_stjepan_forever_card_club_guide_payload($full_user);
    $validation_errors = vip_funnel_collect_payload_validation_errors($payload);
    if(!empty($validation_errors)) {
        error_log('[VIP Funnel] Stjepan FCC guide auto-provision validation failed: ' . vip_funnel_json_encode($validation_errors));
        return null;
    }

    return vip_funnel_studio_create_funnel_from_payload($full_user, $payload);
}
