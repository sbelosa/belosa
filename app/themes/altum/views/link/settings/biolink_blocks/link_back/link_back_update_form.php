...
<form id="<?= 'update_biolink_block_' . $row->biolink_block_id ?>" name="update_biolink_" method="post" role="form" data-type="<?= $row->type ?>">
    ...
    <div class="form-group">
        <label for="<?= 'link_back_fallback_url_' . $row->biolink_block_id ?>"><i class="fa fa-fw fa-link fa-sm text-muted mr-1"></i> <?= l('create_biolink_link_back_modal.fallback_url') ?></label>
        <input id="<?= 'link_back_fallback_url_' . $row->biolink_block_id ?>" type="url" class="form-control" name="fallback_url" value="<?= $row->settings->fallback_url ?? $row->location_url ?>" maxlength="2048" placeholder="<?= l('global.url_placeholder') ?>" />
