...
<form id="<?= 'update_biolink_block_' . $row->biolink_block_id ?>" name="update_biolink_" method="post" role="form" data-type="<?= $row->type ?>">
    ...
    <button class="btn btn-block btn-gray-300 my-4" type="button" data-toggle="collapse" data-target="#<?= 'service_settings_container_' . $row->biolink_block_id ?>" aria-expanded="false" aria-controls="<?= 'service_settings_container_' . $row->biolink_block_id ?>">
        <i class="fas fa-fw fa-credit-card fa-sm mr-1"></i> <?= l('biolink_service.service_header') ?>
    </button>

    <div class="collapse" data-parent="<?= '#update_biolink_block_' . $row->biolink_block_id ?>" id="<?= 'service_settings_container_' . $row->biolink_block_id ?>">
        <div class="form-group">
            <label for="<?= 'service_title_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-sm fa-heading text-muted mr-1"></i> <?= l('biolink_service.title') ?></label>
        ...
        <div class="form-group">
            <label for="<?= 'service_description_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-sm fa-align-left text-muted mr-1"></i> <?= l('biolink_service.description') ?></label>
        ...
        <div class="form-group">
            <label for="<?= 'service_price_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-sm fa-tag text-muted mr-1"></i> <?= l('biolink_product.price') ?></label>
        ...
        <div class="form-group">
            <label for="<?= 'service_minimum_price_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-sm fa-hand-holding-usd text-muted mr-1"></i> <?= l('biolink_product.minimum_price') ?></label>
            <input type="number" min="0" step="0.01" id="<?= 'service_minimum_price_' . $row->biolink_block_id ?>" name="minimum_price" class="form-control" value="<?= $row->settings->minimum_price ?? null ?>" />
        </div>

        <div class="form-group">
            <label for="<?= 'service_currency_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-sm fa-coins text-muted mr-1"></i> <?= l('biolink_service.currency') ?></label>
            <select id="<?= 'service_currency_' . $row->biolink_block_id ?>" name="currency" class="custom-select">
                <?php foreach(get_currencies_array() as $currency_code => $currency_name): ?>
                    <option value="<?= $currency_code ?>" <?= ($row->settings->currency ?? null) == $currency_code ? 'selected="selected"' : null ?>>
        ...
        <div class="form-group custom-control custom-switch">
            <input id="<?= 'service_allow_custom_price_' . $row->biolink_block_id ?>" name="allow_custom_price" type="checkbox" class="custom-control-input" <?= ($row->settings->allow_custom_price ?? null) ? 'checked="checked"' : null?>>
            <label class="custom-control-label" for="<?= 'service_allow_custom_price_' . $row->biolink_block_id ?>"><?= l('biolink_product.allow_custom_price') ?></label>
        </div>

        <div class="form-group">
            <label for="<?= 'service_thank_you_title_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-sm fa-heart text-muted mr-1"></i> <?= l('biolink_donation.thank_you_title') ?></label>
        ...
        <div class="form-group">
            <label for="<?= 'service_thank_you_description_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-sm fa-comment-alt text-muted mr-1"></i> <?= l('biolink_donation.thank_you_description') ?></label>
        ...
        <div class="form-group">
            <label for="<?= 'service_thank_you_url_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-sm fa-link text-muted mr-1"></i> <?= l('biolink_donation.thank_you_url') ?></label>
            <input type="url" id="<?= 'service_thank_you_url_' . $row->biolink_block_id ?>" name="thank_you_url" class="form-control" value="<?= $row->settings->thank_you_url ?? null ?>" placeholder="<?= l('global.url_placeholder') ?>" maxlength="<?= $data->biolink_blocks['product']['fields']['thank_you_url']['max_length'] ?>" />
            <small class="form-text text-muted"><?= l('biolink_donation.thank_you_url_help') ?></small>
        </div>

        <div class="mb-3">
            <div class="d-flex flex-wrap flex-row justify-content-between">
                <label for="<?= 'service_payment_processors_ids_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-sm fa-credit-card text-muted mr-1"></i> <?= l('payment_processors.payment_processors_ids') ?></label>
    ...
    <div class="collapse" data-parent="<?= '#update_biolink_block_' . $row->biolink_block_id ?>" id="<?= 'service_data_container_' . $row->biolink_block_id ?>">
