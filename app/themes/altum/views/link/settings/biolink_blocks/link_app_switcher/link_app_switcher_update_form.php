...
<form id="<?= 'update_biolink_block_' . $row->biolink_block_id ?>" name="update_biolink_" method="post" role="form" data-type="<?= $row->type ?>">
    ...
    <?php
    $available_apps = db()->where('user_id', $this->user->user_id)->where('type', 'biolink')->orderBy('datetime', 'ASC')->get('links', null, ['link_id', 'url']);
    $selected_app = (string) ($row->settings->selected_app ?? '');
    ?>

    <div class="form-group">
        <label for="<?= 'link_app_switcher_selected_app_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-mobile-alt fa-sm text-muted mr-1"></i> <?= l('create_biolink_link_app_switcher_modal.apps') ?></label>
        <select id="<?= 'link_app_switcher_selected_app_' . $row->biolink_block_id ?>" name="selected_app" class="custom-select" required="required">
            <?php foreach($available_apps as $app): ?>
                <option value="<?= $app->link_id ?>" <?= $selected_app === (string) $app->link_id ? 'selected="selected"' : null ?>><?= SITE_URL . $app->url ?></option>
            <?php endforeach ?>
        </select>
