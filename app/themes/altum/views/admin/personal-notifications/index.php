...
<div class="d-flex flex-column flex-md-row justify-content-between mb-4">
    <h1 class="h3 mb-3 mb-md-0 text-truncate"><i class="fas fa-fw fa-xs fa-code-branch text-primary-900 mr-2"></i> <?= l('admin_personal_notifications.header') ?></h1>

    <div class="d-flex position-relative d-print-none">
        <div class="ml-3">
            <a href="<?= url('admin/statistics/personal_notifications') ?>" class="btn btn-gray-300" data-tooltip title="<?= l('global.statistics') ?>">
        ...
        <div class="ml-3">
            <div class="dropdown">
                <button type="button" class="btn btn-gray-300 dropdown-toggle-simple <?= count($data->personal_notifications) ? null : 'disabled' ?>" data-toggle="dropdown" data-boundary="viewport" data-tooltip title="<?= l('global.export') ?>" data-tooltip-hide-on-click>
                    <i class="fas fa-fw fa-sm fa-download"></i>
                </button>

                <div class="dropdown-menu dropdown-menu-right d-print-none">
                    <a href="<?= url('admin/personal-notifications?' . $data->filters->get_get() . '&export=csv')  ?>" target="_blank" class="dropdown-item <?= $this->user->plan_settings->export->csv ? null : 'disabled pointer-events-all' ?>" <?= $this->user->plan_settings->export->csv ? null : get_plan_feature_disabled_info() ?>>
                        <i class="fas fa-fw fa-sm fa-file-csv mr-2"></i> <?= sprintf(l('global.export_to'), 'CSV') ?>
                    </a>
                    <a href="<?= url('admin/personal-notifications?' . $data->filters->get_get() . '&export=json') ?>" target="_blank" class="dropdown-item <?= $this->user->plan_settings->export->json ? null : 'disabled pointer-events-all' ?>" <?= $this->user->plan_settings->export->json ? null : get_plan_feature_disabled_info() ?>>
        ...
        <div class="ml-3">
            <div class="dropdown">
                <button type="button" class="btn <?= $data->filters->has_applied_filters ? 'btn-dark' : 'btn-gray-300' ?> filters-button dropdown-toggle-simple <?= count($data->personal_notifications) || $data->filters->has_applied_filters ? null : 'disabled' ?>" data-toggle="dropdown" data-boundary="viewport" data-tooltip data-html="true" title="<?= l('global.filters.tooltip') ?>" data-tooltip-hide-on-click>
...
<?php if (!empty($data->personal_notifications)): ?>
    <form id="table" action="<?= SITE_URL . 'admin/personal-notifications/bulk' ?>" method="post" role="form">
            ...
            <table class="table table-custom">
                ...
                <?php foreach($data->personal_notifications as $row): ?>
                    ...
                    <tr>
                        ...
                        <td class="text-nowrap">
                            <a href="<?= url('admin/subscribers?subscriber_id=' . $row->subscriber_id) ?>">
                                <?= $row->ip ?>
                            </a>
                        </td>
                        ...
                        <td>
                            <div class="d-flex justify-content-end">
                                <?= include_view(THEME_PATH . 'views/admin/personal-notifications/admin_personal_notification_dropdown_button.php', ['id' => $row->personal_notification_id, 'resource_name' => $row->name, ]) ?>
                            </div>
                        </td>
                    </tr>
                <?php endforeach ?>
