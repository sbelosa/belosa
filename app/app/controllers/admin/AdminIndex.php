...
class AdminIndex extends Controller {
    ...
    public function get_stats_ajax() {
        ...
        /* Custom code: FC-2026-03-05: forever webshop block filters (includes BIH and ALB Kosovo variants) */
        $forever_shop_block_types = ['link_forever_shop', 'link_forever_living_bih', 'link_forever_living_alb_kosovo', 'link_forever_living_albania_kosovo'];
        $forever_all_block_types = array_merge($forever_shop_block_types, ['link_discount']);
        ...
        foreach($period_start_map as $period_key => $period_start_datetime) {
            ...
            while($period_forever_click_row = $period_forever_clicks_result->fetch_object()) {
                ...
                if($period_forever_click_row->type == 'link_discount') {
            ...
            $period_top_registration_sources_result = database()->query("SELECT {$period_source_label_sql} AS `source`, COUNT(*) AS `total` FROM `track_links` LEFT JOIN `biolinks_blocks` ON `track_links`.`biolink_block_id` = `biolinks_blocks`.`biolink_block_id` WHERE `track_links`.`datetime` >= '{$period_start_datetime}' AND `biolinks_blocks`.`type` = 'link_discount' GROUP BY `source` ORDER BY `total` DESC LIMIT 100");
            ...
            if(!empty($period_leaderboard)) {
                foreach(array_map(fn($entry) => (int) $entry['user_id'], $period_leaderboard) as $leaderboard_user_id) {
                    ...
                    while($period_user_forever_click_row = $period_user_forever_clicks_result->fetch_object()) {
                        ...
                        if($period_user_forever_click_row->type == 'link_discount') {
        ...
        if(!empty($leaderboard)) {
            ...
            foreach($leaderboard_user_ids as $leaderboard_user_id) {
                ...
                while($user_forever_click_row = $user_forever_clicks_result->fetch_object()) {
                    ...
                    if($user_forever_click_row->type == 'link_discount') {
