...
class AdminSettings extends Controller {
    ...
    public function main() {
        ...
        if(!empty($_POST)) {
            ...
            $value = json_encode([
                ...
                /* Custom code: FC-2026-03-02: additional user zone lock mode */
                'user_zone_maintenance_is_enabled' => isset($_POST['user_zone_maintenance_is_enabled']),
    ...
    public function fcc_education() {
        $this->process();

        if(!empty($_POST)) {
            /* Custom code: FC-2026-02-24: FCC education page settings */
            $education_by_language = [];
            $education_post = $_POST['education'] ?? [];

            foreach(array_keys((array) \Altum\Language::$active_languages) as $language_name) {
                $language_post = $education_post[$language_name] ?? [];
                $videos_rows = preg_split('/\r\n|\r|\n/', trim($language_post['videos'] ?? ''));
                $videos = [];
                ...
                foreach($videos_rows as $row) {
                    ...
                    $title = $parts[0] ?? '';
                    $id_candidate = $parts[1] ?? $parts[0] ?? '';

                    $vimeo_id = null;
                    if(preg_match('/vimeo\.com\/(?:video\/)?(\d+)/', $id_candidate, $matches)) {
                        $vimeo_id = $matches[1];
                    } elseif(preg_match('/^\d+$/', $id_candidate)) {
                        $vimeo_id = $id_candidate;
                    }
                    ...
                    if($title === $id_candidate) {
                        $title = 'Video';
                    }

                    $videos[] = [
                        'title' => input_clean($title, 128),
                        'vimeo_id' => $vimeo_id,
                    ];
                }

                $education_by_language[$language_name] = [
                    'enabled' => isset($language_post['enabled']) ? 1 : 0,
                    'title' => input_clean($language_post['title'] ?? '', 64),
                    'subtitle' => input_clean($language_post['subtitle'] ?? '', 256),
                    'cta_label' => input_clean($language_post['cta_label'] ?? '', 64),
                    'videos' => $videos,
                ];
            }
            ...
            $help_widget_pages = [
                'links_biolink' => ['pathname' => '/links', 'searchIncludes' => 'type=biolink', 'title' => 'Biolink pages'],
                'links_link' => ['pathname' => '/links', 'searchIncludes' => 'type=link', 'title' => 'Shortened links'],
                'links_file' => ['pathname' => '/links', 'searchIncludes' => 'type=file', 'title' => 'File links'],
                'links_vcard' => ['pathname' => '/links', 'searchIncludes' => 'type=vcard', 'title' => 'Vcard links'],
                ...
                'pixels' => ['pathname' => '/pixels', 'searchIncludes' => null, 'title' => 'Pixels'],
                'projects' => ['pathname' => '/projects', 'searchIncludes' => null, 'title' => 'Projects'],
                'splash_pages' => ['pathname' => '/splash-pages', 'searchIncludes' => null, 'title' => 'Splash pages'],
            ];
            /* /Custom code: FC-2026-02-24 */

            $help_widget_items_by_language = [];
            $help_widget_post = $_POST['help_widget'] ?? [];

            foreach(
                array_keys((array) \Altum\Language::$active_languages) as $language_name
            ) {
                $help_widget_items = [];
                $language_post = $help_widget_post[$language_name] ?? [];

                foreach($help_widget_pages as $key => $page) {
                    $post_item = $language_post[$key] ?? [];
                    $title = trim($post_item['title'] ?? $page['title']);
                    $vimeo_id = trim($post_item['vimeo_id'] ?? '');
                    $description = trim($post_item['description'] ?? '');
                    $extra_html = $post_item['extra_html'] ?? '';
                    $enabled = isset($post_item['enabled']) ? 1 : 0;

                    $help_widget_items[] = [
                        'match' => [
                            'pathname' => $page['pathname'] ?? null,
                            'pathnamePrefix' => $page['pathname_prefix'] ?? null,
                            'searchIncludes' => $page['searchIncludes'],
                        ],
                        'title' => $title,
                ...
                $help_widget_items_by_language[$language_name] = $help_widget_items;
            }

            if(!db()->where('`key`', 'fcc_education')->has('settings')) {
                db()->insert('settings', ['key' => 'fcc_education', 'value' => json_encode([])]);
            }

            $value = json_encode([
                'help_widget_items_by_language' => $help_widget_items_by_language,
                /* Custom code: FC-2026-02-24: FCC education page settings */
                'education_by_language' => $education_by_language,
                /* /Custom code: FC-2026-02-24 */
            ]);

            $this->update_settings('fcc_education', $value);

            /* Custom code: FC-2026-02-24: Refresh language cache */
            \Altum\Language::clear_cache();
            /* /Custom code: FC-2026-02-24 */
        }
    }
