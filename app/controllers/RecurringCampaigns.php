...
class RecurringCampaigns extends Controller {
    ...
    public function index() {
        ...
        $filters = (new \Altum\Filters(['website_id', 'segment', 'user_id'], ['name', 'title', 'description', 'url'], ['recurring_campaign_id', 'name', 'title', 'last_run_datetime', 'next_run_datetime', 'datetime', 'last_datetime', 'total_push_notifications', 'total_sent_push_notifications', 'total_displayed_push_notifications', 'total_clicked_push_notifications', 'total_closed_push_notifications', 'total_campaigns']));
        ...
        if(count($recurring_campaigns) && !$filters->has_applied_filters) {
            ...
            $subscribers_logs_chart = \Altum\Cache::cache_function_result('recurring_campaigns_subscribers_logs?user_id=' . $this->user->user_id, 'user_id=' . $this->user->user_id, function() use ($subscribers_logs_result_query) {
            ...
            switch($_POST['type']) {
                case 'delete':
                    ...
                    foreach($_POST['selected'] as $recurring_campaign_id) {
                        if($recurring_campaign = db()->where('recurring_campaign_id', $recurring_campaign_id)->where('user_id', $this->user->user_id)->getOne('recurring_campaigns', ['recurring_campaign_id'])) {
                            (new RecurringCampaign())->delete($recurring_campaign_id);
                        }
                    }
