...
class PwaCreate extends Controller {
    ...
    public function index() {
        ...
        /* Team checks */
        if(\Altum\Teams::is_delegated() && !\Altum\Teams::has_access('create.pwa')) {
            Alerts::add_error(l('global.info_message.team_no_access'));
            redirect('pwas');
        }

        /* Check for the plan limit */
        $total_rows = database()->query("SELECT COUNT(*) AS `total` FROM `pwas` WHERE `user_id` = {$this->user->user_id}")->fetch_object()->total ?? 0;

        if($this->user->plan_settings->pwas_limit != -1 && $total_rows >= $this->user->plan_settings->pwas_limit) {
        ...
        $values = [
            ...
            'description' => $_POST['description'] ?? '',
            'start_url' => $_POST['start_url'] ?? '',
            'display' => $_POST['display'] ?? 'standalone',
            'orientation' => $_POST['orientation'] ?? 'any',
            'background_color' => $_POST['background_color'] ?? '#000000',
            'theme_color' => $_POST['theme_color'] ?? '#000000',
            'id' => $_POST['id'] ?? '',
        ];
        ...
        $view = new \Altum\View(\Altum\Plugin::get('pwa')->path . 'views/pwa-create/index', (array) $this, true);
